package com.codefilms.assessment;
...
public class App {
	...
	public static void main(String[] args) {
		...
		//simple algorithm to check for duplicate numbers
		
		int[] numbers = {40,22,22,22,35,9,19,82};
	...
	public static void checkDuplicate(int[] numbers) {
		...
		for(int  head = 0, end = tempNumbers.length -1; head < end ;head++)
		{
				...
				if(duplicate != tempNumbers[scanIndex])
			{
				...
				// Prompt user of no duplicate found when the scanIndex is at the end of the container.
				if(scanIndex == tempNumbers.length)
					System.out.println("No duplicate found");
				
			}
				// for a duplicate found, show the scanned number and duplicate number which is the scanned number and the scan index of the duplicate
				else {
					// Tell the user the 
					System.out.println("Scanned Number:"+tempNumbers[scanIndex] + "\nDuplicate:"+duplicate+"\nScanIndex:"+scanIndex);
				}
